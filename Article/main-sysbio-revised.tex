...
% --- bibliography ---
\usepackage{natbib}
% underscore package removed: it makes _ active, which corrupts cite keys
% containing underscores when the .aux file is re-read by natbib.
% hyperref + xurl already handle underscores in URLs.
%
% (multiply-defined fix is applied at the end of the document body)
\usepackage{newpxtext}
\usepackage{newpxmath}
...
To model rate heterogeneity across the phylogeny, we discretise all branches at regular intervals, $\delta t$ \revised{(set to $10^{-3}$~Myr, i.e.\ 1000 years, in all our analyses)} and approximate the GBM described in the previous section. Gibbs sampling is used for most parameters, leveraging conjugate priors to enhance the efficiency of the inference process, and a Metropolis-Hastings algorithm is used for augmenting unobserved branches \citep{quintero_imbalanced_2024, quintero_loss_2025}. The fossil sampling rate ($\psi$) is inferred similarly to the FBD and FBDD processes, using a conjugate Gamma prior updated based on the number of fossils and the total branch length of the complete augmented phylogenies, as in \citet{quintero_loss_2025}. In the following, we detail parts of the inference particular to the OBD and OBDD process, i.e., those involved in the integration of \revised{uncoded} occurrences (sampling rate $\omega$).
...
\revised{The Gamma distribution was chosen as a prior for the sampling rates $\psi$ and $\omega$ because it is conjugate to the Poisson likelihood, enabling closed-form Gibbs updates and faster MCMC mixing. Although empirical studies of fossil sampling heterogeneity among taxa have found support for lognormal distributions \citep{wagner_modelling_2013}, both the Gamma and lognormal are right-skewed unimodal distributions on the positive reals that closely approximate each other in the relevant parameter ranges, so the practical difference is expected to be small.}
...
For the OBD model, the recovery proportions of the true parameter values consistently match the credible interval widths (\ref{fig:cobd-calibration}) and the estimation errors decrease with increasing tree size (\ref{fig:cobd-relerror}). The OBDD model is similarly well calibrated for all parameters (Fig.~\ref{fig:obdd-calibration}), and the estimation errors for both speciation and extinction rates diminish as the tree size grows (Fig.~\ref{fig:obdd-relerror}). Overall, the validation confirms that both models perform reliably in recovering true diversification parameters from simulated data. \revised{Although many of the simulated phylogenies are of moderate size, the purpose of the calibration study is to validate the correctness of the inference algorithm, which, once verified, generalises to larger phylogenies.}
...
\begin{figure}[!ht]
    ...
    \end{subfigure}
    \caption{\textbf{Simulation-based validation of the OBD model.} \revised{Results of the simulation-based calibration study on 1000 datasets simulated under the OBD model. (a)~Truth recovery proportions: the proportion of datasets for which the true parameter value falls within posterior credible intervals of increasing width (from 5\% to 95\%). A well-calibrated model produces points along the diagonal. (b)~Estimation error, measured as the normalized squared error $(\hat{\theta} - \theta)^2 / \theta^2$ (where $\hat{\theta}$ is the posterior median and $\theta$ the true value), plotted against the number of tips in the reconstructed phylogeny.}}
...
Importantly, the fossil record is inherently incomplete, limiting our direct observation of past diversity. \revised{Several non-phylogenetic approaches also account for unsampled lineages when estimating richness or diversification rates, including three-timers \citep{alroy_dynamics_2008}, capture-mark-recapture methods \citep{connolly_joint_2001}, forward and reverse survivorship \citep{foote_inferring_2001}, and TRiPS \citep{starrfelt_how_2016}.} \revised{The phylogenetic modeling framework, including all models presented here, complements these approaches by using phylogenetic relationships to inform the placement and timing of unsampled lineages. Ghost lineages (branches implied by the tree topology between divergence points) are accounted for by the phylogenetic reconstruction itself. In addition, the data augmentation procedure simulates Signor-Lipps extensions (extending known ranges forward in time) and entirely unsampled lineages that originate, diversify, and go extinct without leaving any observed trace (see Fig.~\ref{fig:augmented-tree}).}
...
\revised{From a methodological standpoint, the data augmentation approach used here simplifies the likelihood computation by conditioning on complete (augmented) trees, making it straightforward to extend the model, for instance to incorporate rate heterogeneity or piecewise sampling, without rederiving the likelihood analytically. This flexibility comes at the cost of an expanded state space that the MCMC must explore, so data augmentation is best suited to settings where the augmented data remain tractable.}

Looking ahead, the OBDD model opens new avenues. \revised{Uncoded} occurrences can often be attributed to specific subclades, and incorporating this information -- similar to the use of topological constraints in traditional FBD analyses \citep{baridosottani_putting_2023} -- would provide more direct insights into lineage-specific rates. In some cases, \revised{uncoded} occurrences might be identified at the morphospecies level, offering even finer resolution \citep{silvestro_bayesian_2014, stadler_fossilized_2018}.  Additionally, model accuracy may be enhanced by coupling the \revised{coded} fossil sampling rate with the \revised{uncoded} occurrence sampling rate instead of inferring them independently, as these rates are expected to partially co-vary. These fossil sampling rates could also be allowed to evolve gradually along branches of the phylogeny, capturing variations in sampling across species and extending existing models with heterogeneous sampling processes \citep{barido-sottani_integrating_2025}.  These developments augur an exciting trajectory for models that, with ever-greater data integration and flexibility, can gradually uncover more subtle complexities of past evolutionary processes.

\section{Acknowledgments}

\revised{We thank the Associate Editor (Dr.\ April Wright), Dr.\ Peter Wagner, and two anonymous reviewers for their constructive comments.} The authors thank the members of the Morlon team for their feedback on an earlier version of this manuscript. \textcolor{red}{\textbf{[TODO: Acknowledge top PBDB data contributors for cetacean records and obtain a PBDB publication number. See https://paleobiodb.org/\#/ for contributor list.]}}
