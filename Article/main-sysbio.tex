...
% --- bibliography ---
\usepackage{natbib}
\usepackage[strings]{underscore} % <-- fixes bare '_' in bibliography
\usepackage{newpxtext}
\usepackage{newpxmath}
...
\begin{figure}[!ht]
    ...
    \caption{\textbf{The Occurrence Birth-Death Diffusion process.} This figure illustrates a simulation under the OBDD process. One key feature is that speciation and extinction rates are not specified in advance, except for starting values (resp. $\lambda_0$ and $\mu_0$), but instead change gradually along each new branch of the phylogeny. The other important feature is that the sampling process produces fossil occurrences that are not placed in the phylogeny, in addition to the standard phylogenetic fossils. Consequently, each simulation will result in both a sample of rate trajectories (right column, middle) and a phylogeny with fossils and a record of occurrences (right column, resp. brown diamonds and orange circles).}
...
The simpler OBD model with constant homogeneous speciation and extinction rates is also implemented in Tapestree.jl \citep{quintero_tapestreejl_2020} but as an independent model, extending the constant-rate FBD model \citep{quintero_rise_2025}. The integration of occurrences relies on the same functions for the Gibbs sampling and Metropolis-Hastings steps described above.
...
Regarding the influence of tree size on computation time, Figure~\ref{fig:running-time} shows that, for datasets generated under the OBDD model and for a fixed number of MCMC iterations, our inference should take an extra day for each additional 150 tips in the reconstructed phylogeny (including fossil tips). The Generalised Linear Model (GLM) and Generalised Additive Model (GAM) regressions (see Appendix \ref{SM-runtime}) revealed no influence of the number of fossils or occurrences on the runtime, as apparent in Figure~\ref{fig:running-time}, indicating that the code has been adequately optimised for extensive fossil integration.
...
\bibliographystyle{sysbio}
{
  ...
  \bibliography{OBDD_Cetacea} % exact .bib filename (case-sensitive)
}
