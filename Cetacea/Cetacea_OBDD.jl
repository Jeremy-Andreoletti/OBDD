...
f_epoch  = Int64[0]
niter    = veryShortMCMC ? 100 : (shortMCMC ? 50_000 : 10_000_000)
...
ofile    = "OBDD_Cetaceans_rateαprior0.1_$(niter)iter_seed$(seed_nb)" ; isdir("outputs/") || mkdir("outputs/")
tune_int = 100
ϵi       = 0.2
λi       = NaN
μi       = NaN
...
σλi      = 0.1
σμi      = 0.1
pupdp    = (0.02, 0.02, 0.1, 0.01, 0.01, 0.02, 0.1, 0.5)
δt       = 1e-3
survival = true
mxthf    = 0.05
prints   = 5
...
#out_trees = reduce(vcat, [iread("outputs/OBDD_Cetaceans_rateαprior0.1_$(niter)iter_seed$(seed_nb).txt") for seed_nb in 0:4])
